...
# Start spark Session
spark = SparkSession.builder.appName("correlation_africa").getOrCreate()
...
reordered_df = reordered_df.filter(col("Region") == "Africa")
...
correlation_spark_df.coalesce(1).write.mode("overwrite").option("header", "true").csv("project/final/q3_correlation/africa")