...
# Start spark Session
spark = SparkSession.builder.appName("correlation_europe").getOrCreate()
...
reordered_df = reordered_df.filter(col("Region") == "Europe")
...
correlation_spark_df.coalesce(1).write.mode("overwrite").option("header", "true").csv("project/final/q3_correlation/europe")